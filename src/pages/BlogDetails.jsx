...
export default function BlogDetails() {
  ...
  const fetch = async () => {
    try {
      ...
      if (!blogData.category) console.warn('Blog missing category:', blogData._id);
      setBlog(blogData);
      const all = await getBlogs();
      const allBlogs = all?.data || [];
      const rel = allBlogs.filter(b => b?._id !== id && b?.category && blogData?.category && b.category === blogData.category).slice(0, 3);
      setRelated(rel.length ? rel : allBlogs.filter(b => b?._id !== id).slice(0, 3));
    } catch (e) {
      setError(e.message);
    } finally {
      setLoading(false);
    ...
    <div className="w-full px-8 sm:px-12 lg:px-16 xl:px-24 2xl:px-32 py-8 bg-paper">
      ...
      <div className="bg-white rounded-[32px] overflow-hidden border-2 border-ink-900 shadow-hard">
        <div className="relative">
          <img src={blog.coverImage || 'https://images.unsplash.com/photo-1633356122544-f134324a6cee?w=800&auto=format&fit=crop'} alt={blog.title} className="w-full h-64 sm:h-80 lg:h-[520px] object-cover" />
          ...
          <div className="absolute bottom-4 left-4 right-4 sm:bottom-6 sm:left-6 sm:right-6 flex flex-wrap gap-2">
            <span className="px-3 py-1.5 rounded-full bg-mustard text-ink-900 text-xs font-black uppercase tracking-widest border-2 border-ink-900 shadow-hard-sm">{blog.category || 'General'}</span>
            <span className="px-3 py-1.5 rounded-full bg-white text-ink-900 text-xs font-bold border-2 border-ink-900 flex items-center gap-1.5"><Clock className="w-3.5 h-3.5" />{blog.readTime || '5 min read'}</span>
